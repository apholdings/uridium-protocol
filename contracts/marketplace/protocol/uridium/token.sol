...
contract Uridium is ERC20, ERC20Burnable, Pausable,ReentrancyGuard, AccessControl, ERC20FlashMint, ERC20Permit {
    ...
    function burn(uint256 wad) public override whenNotPaused {
        require(wad > 0, "Uridium: amount must be greater than zero");
        _burn(msg.sender, wad);
    }

    // --- Alias ---
    function push(address guy, uint wad) external { // Pushes tokens to user
        transferFrom(msg.sender, guy, wad);
    }

    function pull(address guy, uint wad) external { // Pulls tokens from user
