...
contract Vat {
    ...
    mapping (address => uint256)                   public galr;  // [rad]
    ...
    function move(address src, address dst, uint256 rad) external {
        require(wish(src, msg.sender), "Vat/not-allowed");
        galr[src] = _sub(galr[src], rad);
        galr[dst] = _add(galr[dst], rad);
    }
    ...
    function heal(uint rad) external {
        ...
        galr[u] = _sub(galr[u], rad);
        vice   = _sub(vice,   rad);
        debt   = _sub(debt,   rad);
    }
    function suck(address u, address v, uint rad) external auth {
        sin[u] = _add(sin[u], rad);
        galr[v] = _add(galr[v], rad);
        vice   = _add(vice,   rad);
        debt   = _add(debt,   rad);
    }
